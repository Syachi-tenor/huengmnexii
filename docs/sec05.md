...
[sec05.zip](sec05.zip)をダウンロードし，`sec05.zip`があるディレクトリにて `unzip sec05.zip` を実行すれば展開される(今いるディレクトリに展開されるので注意すること; 個別にダウンロードしてもよいし GitHub から取り出したものを利用してもよい)．
...
以下は投稿があった際にその内容を表示する Python プログラムである．
前述の `sec05.zip` に含まれている．
...
`GET` 用の関数で，以下のように引数を定義すると，それが Query String として利用できるようになる．たとえば Query String `from_id=10` で API アクセスとすると，`10` が `from_id` に代入される．
...
また，`messages.messages` とか書くのがややこしいので，これまでの `Messages` は `System` という型に名前を変えた．
